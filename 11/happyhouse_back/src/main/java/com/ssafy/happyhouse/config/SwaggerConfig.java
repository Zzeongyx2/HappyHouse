...
public class SwaggerConfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("happyhouseVueJS").select()
				.apis(RequestHandlerSelectors.basePackage("com.ssafy.happyhouse.controller")).paths(regex("/happyhouse/.*")).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("HAPPYHOUSE VueJS REST API").description(
				"<h2>HAPPYHOUSE API Reference for Developers</h2>Swagger를 이용한  VUEJS API<br><img src=\"/ssafy.png\">\"")
