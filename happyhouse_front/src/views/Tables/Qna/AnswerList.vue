  ...
  <div>
    <b-card no-body>
      ...
      <el-table
        ...
        :data="paginatedItems"
      >
        <el-table-column label="comment" prop="comment" min-width="140px">
        </el-table-column>
      ...
      <form action="">
        <b-row class="form-group">
          ...
          <b-col md="2">
            <b-button variant="outline-light" @click="registerAnswer"
              >답변 추가</b-button
            >
          </b-col>
        </b-row>
    ...
    <b-modal
      v-if="userInfo.userid === answer.userid"
      id="my-modal"
      title="답변 수정"
      @ok="deleteAnswer"
      @cancel="modifyAnswer"
    ...
    modifyAnswer() {
      if (this.comment != "") {
        ...
        modifyAnswer(
          ...
          ({ data }) => {
            if (data === "success") {
              Swal.fire("Modified!", "수정되었습니다.", "success");
            } else {
              Swal.fire("Failed!", "수정 중 문제가 발생하였습니다.", "warning");
            }
            // 현재 route를 /list로 변경.
            this.$router.go();
          },
        ...
        Swal.fire("Failed!", "답변을 입력하세요.", "warning");
      }
    },
    registerAnswer() {
